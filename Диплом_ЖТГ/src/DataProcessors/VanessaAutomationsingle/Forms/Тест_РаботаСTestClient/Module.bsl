...
Функция ЯЗакрываюСеансTESTCLIENT() Экспорт
	...
	Попытка
		Ванесса.ВыполнитьПроверкуСерверныхВызововВОбработчикахСобытий();
	Исключение
	...
	Попытка
		...
		Если КонтекстСохраняемый.Свойство("ТекущийПрофильTestClient") Тогда
			ТекстСообщения = ТекстСообщения + Символы.ПС + Ванесса.Локализовать("Имя:") + " " + КонтекстСохраняемый.ТекущийПрофильTestClient;
		КонецЕсли;	
