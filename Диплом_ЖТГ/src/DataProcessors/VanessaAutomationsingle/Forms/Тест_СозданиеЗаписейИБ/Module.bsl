...
//@ЯУдаляюВсеЭлементыТаблицы(Парам01)
...
Функция НайтиЗаписьТаблицы(ВидОбъекта, ИмяОбъекта, ЗначенияПоиска, ВыбираемыеПоля)
	...
	ОбъектДляПроверки = СоздатьОбъект(ВидОбъекта, ИмяОбъекта);
	
	Для Каждого Значение Из ЗначенияПоиска Цикл 
		
		Если ТипЗнч(ОбъектДляПроверки[Значение.Ключ]) = Тип("Строка") Тогда
			ДлиннаЗначения = СтрДлина(Значение.Значение);
			Если ДлиннаЗначения = 0 Тогда
				// Проверка на пустую строку
				Условия.Добавить(СтрЗаменить("ВЫРАЗИТЬ(%1 КАК СТРОКА(1)) = """"", "%1", Значение.Ключ));
			Иначе
				Условия.Добавить(СтрЗаменить("ВЫРАЗИТЬ(%1 КАК СТРОКА("+Формат(ДлиннаЗначения,"ЧГ=")+")) = &%1", "%1", Значение.Ключ));
			КонецЕсли;	
