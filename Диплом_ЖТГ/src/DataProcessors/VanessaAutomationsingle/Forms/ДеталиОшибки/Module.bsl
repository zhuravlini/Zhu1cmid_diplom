...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДанныеОшибки") Тогда 
		Если ТипЗнч(Параметры.ДанныеОшибки) = Тип("Структура")  Тогда
			...
			Иначе
				Элементы.ФормаАктуализироватьЗначение.Видимость = Истина; //Позволим актуализировать текстовые значения
	      	КонецЕсли;	 
		...
		КонецЕсли;	 
	КонецЕсли;	 
	Если НЕ (Параметры.Свойство("ВозможнаАктуализацияШагаВРедакторе") И Параметры.ВозможнаАктуализацияШагаВРедакторе) Тогда
		Элементы.ФормаАктуализироватьЗначение.Видимость = Ложь;
	КонецЕсли;	
...
&НаКлиенте
Функция ДанныеЭлементаФормыПоИмени(ИмяЭлемента) Экспорт
	ДанныеЭлемента = Новый Структура;
	Попытка
		ЗаголовокЭлемента = Элементы[ИмяЭлемента].Заголовок;
	Исключение
	...
	Попытка
		ДанныеЭлемента.Вставить("ИмяКоманды", Элементы[ИмяЭлемента].ИмяКоманды);
	Исключение
	КонецПопытки;
	ДанныеЭлемента.Вставить("ИсточникДанных", ЭтаФорма);
...
Процедура ПерейтиКСтрокеДереваСОшибкой(Команда)
	Если НомерСтрокиДерева > 0 Тогда
		...
		ДопПараметры.Вставить("НомерСтрокиДерева", НомерСтрокиДерева);
		ДопПараметры.Вставить("УникальныйИдентификатор", Ванесса.УникальныйИдентификатор);
		Оповестить("ПерейтиКСтрокеДереваСОшибкой", ДопПараметры);
	КонецЕсли;	 
...
&НаКлиенте
Процедура АктуализироватьЗначение(Команда) 
	
	ПараметрыАктуализации = Новый Структура;
	ПараметрыАктуализации.Вставить("НомерСтрокиДерева", НомерСтрокиДерева);
	Ванесса.АктуализироватьПоДаннымОшибки(ПараметрыАктуализации);
	Закрыть();
