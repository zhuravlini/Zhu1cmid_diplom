...
&НаКлиенте
Функция ОчиститьКаталогиОтчетов()
...
Функция ЯЗагружаюСписокФичаФайловИзУказанныхНастроекИФильтров() Экспорт
...
Функция ЯЗапускаюНесколькоСеансовРаннеровСогласноУказанныхНастроек() Экспорт
...
Функция ЯСделалАктивнымСледующийФичаФайлИзСписка() Экспорт
...
&НаКлиенте
Функция ОбновитьСтатусВыполненияФич()
...
Функция ЯЗагрузилВРаннерТекущийФичаФайлИЗапустилНаВыполнение() Экспорт
	...
	Если ОкноVB = Неопределено Тогда
		...
		ДиапазонПортов = Формат(Порт1, "ЧГ=; ЧН=0") + "-" + Формат(Порт2, "ЧГ=; ЧН=0");
...
Функция ЯЗакрываюСеансПовисшегоРаннераИВсехЕгоTestClient() Экспорт
...
Функция ВГлобальномМассивеФайловТогда(ИмяПеременнойМассива,Количество) Экспорт
...
Функция ЯСобралИзРаннераТекущиеДанные() Экспорт
...
Функция ПокаЕстьФичаФайлыДляВыполненияЯВыполняюДействия() Экспорт
...
Функция ДляКаждогоРаннераИзТаблицыРаннеровЯДелаю() Экспорт
	//Возврат Неопределено;
...
Функция ТекущийРаннерСвободенТогда() Экспорт
	ИмяФайлаФлага = КонтекстСохраняемый.МассивРаннеров[Контекст.ИдТекущегоРаннера].ИмяФайлаФлага;
...
Функция ЯЖдуКогдаВсеРаннерыЗакончатВыполнениеСценариев() Экспорт
...
Функция ЯВызываюПроцедуруVanessaBehavior(ИмяПроцедуры) Экспорт
